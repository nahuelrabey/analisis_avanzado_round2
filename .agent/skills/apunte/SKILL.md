...
- Clasificar el tipo de concepto:
  ...
  - **Corolario** (`#corolario[...]`)
  - **Axioma** (`#axioma[...]`)
- **REGLA ESTRICTA:** Si la imagen es un **Ejemplo** (numerado o no) de `notas_materia.pdf` o de cualquier otra fuente, **no usar esta skill**: delegar a la skill `ejemplo` (`apuntes-typst/ejemplos/p{N}.typ`), aunque continúe la numeración por capítulo de un apunte ya cargado. Los ejemplos no van en `apuntes.typ`.
