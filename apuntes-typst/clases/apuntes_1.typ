...
Por arquimedianidad $exists m in NN$ tal que $1/m < (epsilon/3)^2$. Entonces $1/sqrt(m) < epsilon/3$
...
Por el teorema anterior, $(a_n)_(n in NN)$ converge. Para calcular el límite usamos que
...
Tenemos dos opciones, $L = 0$ o $x = 1$. Si $x = 1$, entonces $a_n = x^(n) = 1^n ->_(n->oo) 1$. En cualquier caso, el límite existe.
...
Pero $x^(n+1) - x^n = x^(n) (x-1) >= 0$ ya que $x^n >= 0$, $x-1 > 0$. Si $(a_n)$ convergiera, repitiendo el argumento de tomar el límite en (\*) nos daría que $x = 1$ o $L = 0$. Como $x > 1$, debe ser $L = 0$. Pero una sucesión *creciente* y convergente converge al supremo del conjunto de sus términos, por lo tanto $a_n = x^n <= L = 0$ para todo $n in NN$, lo cual es absurdo (pues $x^n > 1 > 0$), y por lo tanto no converge.
...
Pero la diferencia entre dos enteros distintos es, al menos, $1$. Concluimos que $a_n = a_(n_1)$ para todo $n >= n_1$. Como $a_n$ converge, $lmn a_n = a_(n_1) = L$.

Podemos usar sucesiones para calcular supremos e infimos

_Lema_: Sea $A subset.eq RR$ no vacío y acotado superiormente. Tenemos que $s = sup A$ sii:
...
+ $1/n + m >1$ para todo $n,m in NN$. Como $a_n = 1/n + 1$ está en $A$ y cumple que $lmn a_n = 1$, concluimos que 1 es el ínfimo de $A$.
...
+ $A$ está acotado superiormente por $x$, así que por completitud existe $n = sup A$. Es este $n$ el que vamos a ver que cumple lo pedido.
...
+ $n in ZZ$: como es el supremo, existe $(a_k)_k subset.eq A$ tal que $lim_(k->+oo) a_k = n$. Pero, por el _Ejemplo 3_, existe $k_0 in NN$ tal que $a_k = n$ para todo $k >= k_0$. Entonces $n in ZZ$
...
_Ejemplo 6_: Probar que para todo $x in RR$ existe $(q_n)_n subset.eq QQ$ estrictamente decreciente tal que $lmn q_n = x$.
...
(2) $lmn q_n = x$. Como $(q_n)_n$ es decreciente y acotada inferiormente por $x$, es convergente a $q in RR$
